...
import (
	...
	"github.com/webermarci/go-piface/ioctl"
)
...
func (spi *SPIDevice) Open() error {
	...
	if err != nil {
		return fmt.Errorf("%s is not found. have you enabled the SPI module? (%s)", spiDevice, SPI_HELP_LINK)
	}
...
func (spi *SPIDevice) Send(bytes_to_send [3]byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error on sending: %s", err)
	}
...
func (spi *SPIDevice) SetBitsPerWord(bpw uint8) error {
	...
	if err != nil {
		return fmt.Errorf("error setting bits per word: %s", err)
	}
	return nil
}
...
func (spi *SPIDevice) SetSpeed(speed uint32) error {
	...
	if err != nil {
		return fmt.Errorf("error setting speed: %s", err)
	}
	return nil
}
